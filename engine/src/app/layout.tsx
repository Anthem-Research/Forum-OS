...
import { currentActorId, deploymentMode, forumRepository } from "@/core/repository";
import "./globals.css";
...
export default async function RootLayout({ children }: Readonly<{ children: ReactNode }>) {
  ...
  await connection();
  const actor = forumRepository.userById(currentActorId());
  const node = actor?.nodeId ? forumRepository.nodeById(actor.nodeId) : undefined;
  return (
    <html lang="en">
      <body>
        <ForumHeader nodeName={node?.name ?? "Forum"} mode={deploymentMode()} />
        {children}
