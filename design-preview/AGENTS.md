...
## Forum OS Product Decisions

- The explicit target is the selected "Quiet Axis" landscape launcher for a Samsung Galaxy Tab S10 Lite.
- The child-facing home surface contains only a live clock, date, `FORUM`, and the five actions `ASK`, `DRAW`, `LOOK`, `SYNTHESIS`, and `PLAY`.
- It is a quiet institutional instrument, not a conventional children’s UI: no icons, illustrations, cards, widgets, dock, drawer, feed, badges, notifications, or visible Android machinery.
- Parent controls are reached only by a concealed long-press on `FORUM`, followed by a PIN gate. This must never add visible child-facing chrome.
- Preserve the selected palette: near-black forest `#0D1C16`, warm paper `#F5F1E6`, and a single Forum orange accent `#EE6B3B`.
- A Galaxy Tab landscape preview preset is an explicit product requirement. Keep the original iPhone and Pixel presets intact while adding it.
